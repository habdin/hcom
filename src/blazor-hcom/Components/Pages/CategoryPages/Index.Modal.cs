...
public partial class Index : ComponentBase, IAsyncDisposable
{
	...
	private Modal<Category>? _myModalRef;
	...
	private RenderFragment MainForm(string operation) => operation switch
	{
		"create" or "update" => builder =>
		{
			...
			builder.AddAttribute(1, "Category", Item);
	...
	private EditContext CreateEditContext(Category item) => new EditContext(item);

	// Helper methods used to tweak Modal title.
	// GetModalTitle depends on GetTitle.

	private string GetTitle(Category item) => item?.Name ?? "";
	private string GetModalTitle(string crudOper, Category item)
	{
	...
	private async Task HandleSubmit(Category item, string? operation)
	{
		// [HCOM-DESIGN NOTE]
		// HandleSubmit() = central submission handler for Category modal.
		...
		// This guarantees the Category page stays synchronized with backend state.
