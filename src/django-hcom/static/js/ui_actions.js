...
export class ComponentInitializer {
        elementInitializer({
                ...
                if (!this[component]) {
                        if (!elementId && isDevelopment) {
                                console.log("the component {elementname is not present on this page.}");
                        } else {
                                const element = document.getElementById(elementId);
                                if (element) {
                                        // log the success of the element initialization when the app is in development state
                                        const result = typeof customLogic === "function" ? customLogic(element) : element;
                                        this[component] = result;
                                        if (isDevelopment && this[component]) { console.log(`${elementName} was successfully initialized.`); }
                                }
                        }
                }
        ...
        batchInitialize(configs) {
                ...
                configs.forEach(config => {
                        const instance = this.elementInitializer(config);
                        results[config.component] = instance;
                });
                return results;
        }
}
...
export class CRUDSManager {
        ...
        constructor(baseURL) {
                ...
                // This snippet is used to log the initialization of the class using the constructor method.
                if (isDevelopment) {
                ...
                const components = [
                        {
                                // For Modal Initialization
                                component: "Modal", elementId: "theModal", elementName: "Modal",
                                customLogic: (element) => bootstrap.Modal.getOrCreateInstance(element)
                        },
                        {
                                // For Viewtype Initialization
                ...
                const initialized = CompInit.batchInitialize(components);
                // components.forEach(config => {
                //     const key = config.component;
                //     if (!initialized[key]) {
                //         console.error('Component "${config.elementName}" failed to initialize!');
                //     }
                // })
                ...
                if (this.Modal) {
                        ...
                        this.Modal.show = this.Modal.show.bind(this.Modal);
